...
make -C "${GLUONDIR}" GLUON_TARGET=${GLUON_SINGLE_TARGET} DEVICES="${GLUON_SINGLE_DEVICE}" GLUON_BRANCH="${TARGET_BRANCH}" -j"$(nproc || echo 1)" V=s

make -C "${GLUONDIR}" GLUON_BRANCH="${TARGET_BRANCH}" BROKEN=1 V=s manifest
